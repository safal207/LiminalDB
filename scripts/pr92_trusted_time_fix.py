#!/usr/bin/env python3
"""Apply the one-shot PR #92 trusted-time lifecycle correction."""
...
CHECKPOINT = Path("liminal-db/crates/liminal-store/src/checkpoint.rs")
FIXTURE = Path(
    "liminal-db/crates/liminal-store/tests/fixtures/"
    "signed-checkpoint-anti-rollback-v0.1.json"
)
DOCS = Path("docs/integrations/SIGNED_CHECKPOINT_ANTI_ROLLBACK_V0_1.md")
...
def patch_fixture() -> None:
    ...
    next(key for key in fixture["keys"] if key["name"] == "trusted-old")[
        "valid_until_ms"
    ] = 500
    if not any(key["name"] == "expired" for key in fixture["keys"]):
        fixture["keys"].insert(
            3,
            {
                "name": "expired",
    ...
    if "expired_key_rejected_at_trusted_time" not in case_ids:
        index = next(
            ...
            if case["scenario"] == "stale_checkpoint"
        )
        fixture["cases"][index:index] = [
            {
                "case_id": "expired_key_rejected_at_trusted_time",
                "scenario": "expired_key",
                "expected": "KEY_EXPIRED",
            },
            {
                "case_id": "future_issued_checkpoint_rejected",
                "scenario": "future_issued_checkpoint",
                "expected": "CHECKPOINT_FROM_FUTURE",
            },
        ]
