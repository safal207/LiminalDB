...
pub fn replace_snapshot_bytes_crash_safe(
    ...
    trigger_durability_failpoint(
        DurabilityFailpoint::SnapshotAfterRenameBeforeDirectorySync,
    );
    sync_directory(parent).map_err(storage_error)?;
...
pub fn recover_interrupted_snapshot_replace(
    snapshot_path: &Path,
) -> Result<(), std::io::Error> {
...
fn parent(path: &Path) -> Result<&Path, TransitionLedgerError> {
    path.parent().ok_or_else(|| {
        TransitionLedgerError::Storage("snapshot path has no parent".to_owned())
    })
}
...
fn sibling(path: &Path, suffix: &str) -> Result<PathBuf, std::io::Error> {
    ...
    let name = path.file_name().and_then(|value| value.to_str()).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::InvalidInput, "invalid snapshot filename")
    })?;
